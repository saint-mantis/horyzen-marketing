...
(function() {
    ...
    window.integro_remove_observer = function(id) {
        if (typeof window.trx_addons_remove_observer != 'undefined') {
            ...
            if (typeof integro_observers[id] !== 'undefined') {
                integro_observers[id].observer.disconnect(integro_observers[id].obj);
    ...
    window.integro_import_inline_styles = function(response) {
        var selectors = integro_apply_filters('integro_filter_spa_inline_css_selectors', ['integro-inline-styles-inline-css', 'trx_addons-inline-styles-inline-css', 'woocommerce-inline-inline-css', 'elementor-frontend-inline-css']);
    ...
    window.integro_set_storage = function(name, value) {
        if (integro_is_local_storage_exists()) window['localStorage'].setItem(name, value);
        else integro_set_cookie(name, value, 365 * 24 * 60 * 60 * 1000);
    };
    window.integro_del_storage = function(name) {
        if (integro_is_local_storage_exists()) window['localStorage'].removeItem(name);
        else integro_del_cookie(name);
    };
    window.integro_clear_storage = function() {
        if (integro_is_local_storage_exists()) window['localStorage'].clear();
    };
    ...
    window.integro_adminbar_height_calc = function() {
        return integro_apply_filters('integro_filter_adminbar_height', $adminbar.length === 0 || $adminbar.css('display') == 'none' || $adminbar.css('position') == 'absolute' ? 0 : $adminbar.height());
    };
    ...
    window.integro_fixed_rows_height = function() {
        ...
        return typeof trx_addons_fixed_rows_height != 'undefined' ? trx_addons_fixed_rows_height(with_admin_bar, with_fixed_rows) : (with_admin_bar ? integro_adminbar_height() : 0);
    };
...
jQuery(document).ready(function() {
    ...
    function integro_ready_actions() {
        $body.on('focus', 'a.integro_skip_link', function() {
    ...
    function integro_add_select_container(cont) {
        ...
        var selector = integro_apply_filters('integro_filter_select_container_selector', 'select:not(.esg-sorting-select):not([class*="trx_addons_attrib_"]):not(#rating)');
        if (selector) {
    ...
    function integro_init_media_elements(cont) {
        var audio_selector = integro_apply_filters('integro_filter_mediaelements_audio_selector', 'audio:not(.inited)'),
            video_selector = integro_apply_filters('integro_filter_mediaelements_video_selector', 'video:not(.inited):not([nocontrols]):not([controls="0"]):not([controls="false"]):not([controls="no"])'),
        ...
        if (INTEGRO_STORAGE['use_mediaelements'] && cont.find(media_selector).length > 0) {
            if (window.mejs) {
                ...
                cont.find(media_selector).each(function() {
                    ...
                    if (!$self.hasClass('no-mejs') && !$self.hasClass('no-mediaelement') && !$self.hasClass('wp-block-cover__video-background') && $self.addClass('inited').parents('.mejs-mediaelement').length === 0 && $self.parents('.wp-block-video').length === 0 && $self.parents('.wp-block-media-text').length === 0 && $self.parents('.elementor-background-video-container').length === 0 && $self.parents('.elementor-widget-video').length === 0 && $self.parents('.with_video_autoplay').length === 0 && $self.parents('.sc_layouts_title').length === 0 && (INTEGRO_STORAGE['init_all_mediaelements'] || (!$self.hasClass('wp-audio-shortcode') && !$self.hasClass('wp-video-shortcode') && !$self.parent().hasClass('wp-playlist')))) {
    ...
    function integro_tabs_ajax_content_loader(panel, page, oldPanel) {
        ...
        jQuery.post(INTEGRO_STORAGE['ajax_url'], {
            ...
            try {
                ...
                rez = {
                    error: INTEGRO_STORAGE['msg_ajax_error']
                };
            ...
            if (rez.error !== '') {
                ...
                panel.prepend(rez.data).fadeIn(function() {
                    ...
                    setTimeout(function() {
                        panel.find('.integro_tab_holder,.integro_tab_content_remove').remove();
                        $window.trigger('scroll');
                    }, 300);
                });
        ...
        cont.find('.nav-load-more:not(.inited)').addClass('inited').on('click', function(e) {
            ...
            if (panel.length === 0) {
                jQuery.get(location.href, {
                    ...
                    if ($response_posts_container.length > 0) {
                        integro_loadmore_add_items($content.find('.posts_container').eq(0), $response_posts_container.find('> .masonry_item,' + '> div[class*="column-"],' + '> article'));
                    }
                    $document.trigger('action.got_ajax_response', {
                ...
                jQuery.post(INTEGRO_STORAGE['ajax_url'], {
                    ...
                    if (rez.error !== '') {
                        ...
                        integro_loadmore_add_items(panel.find('.posts_container'), jQuery(rez.data).find('> .masonry_item,' + '> div[class*="column-"],' + '> article'));
                    }
                    $document.trigger('action.got_ajax_response', {
                        action: 'integro_ajax_get_posts',
                        result: rez,
            ...
            function integro_loadmore_add_items(container, items) {
                if (container.length > 0 && items.length > 0) {
                    ...
                    if (container.hasClass('masonry_wrap')) {
                        ...
                        integro_when_images_loaded(just_loaded_items, function() {
    ...
    function integro_init_load_single_post(cont) {
        cont.find('.posts_container:not(.inited_open_full_post),.sc_blogger_content.sc_item_posts_container:not(.inited_open_full_post)').addClass('inited_open_full_post').on('click', 'a', function(e) {
            ...
            if (link_url && post_url && link_url == post_url && (posts_container.hasClass('open_full_post') || INTEGRO_STORAGE['open_full_post']) && !posts_container.hasClass('no_open_full_post') && !posts_container.hasClass('columns_wrap') && !posts_container.hasClass('masonry_wrap') && posts_container.find('.sc_blogger_grid_wrap').length === 0 && posts_container.find('.masonry_wrap').length === 0 && posts_container.parents('.wp-block-columns').length === 0 && (posts_container.parents('.wpb_column').length === 0 || posts_container.parents('.wpb_column').eq(0).hasClass('vc_col-sm-12')) && (posts_container.parents('.elementor-column').length === 0 || posts_container.parents('.elementor-column').eq(0).hasClass('elementor-col-100'))) {
                ...
                jQuery.get(integro_add_to_url(post_url, {
    ...
    function integro_infinite_scroll_in_single() {
        ...
        var container = INTEGRO_STORAGE['which_block_load'] == 'article' ? $content.eq(0) : $page_content_wrap.eq(0),
        ...
        $single_post_scrollers.each(function() {
            ...
            if (inf.hasClass('nav-links-single-scroll-loaded')) {
                ...
                jQuery.get(integro_add_to_url(link, {
                    ...
                    var $response = jQuery(response),
                        ...
                        $response_post_content = INTEGRO_STORAGE['which_block_load'] == 'article' ? $response_content : $response_page_content_wrap;
                    if ($response_post_content.length > 0) {
                        var html = $response_post_content.html(),
                            response_body_classes = INTEGRO_STORAGE['which_block_load'] == 'article' ? null : response.match(/<body[^>]*class="([^"]*)"/);
                ...
                $i_agree.each(function() {
                    ...
                    form.find('button,input[type="submit"]').on('click', function(e) {
                        if (!chk.get(0).checked) {
                            ...
                            form.append('<div class="trx_addons_message_box trx_addons_message_box_error">' + INTEGRO_STORAGE['msg_i_agree_error'] + '</div>');
            ...
            if (content.css('float') == 'none') {
                ...
                if (sb_height < content_height && integro_window_scroll_top() + integro_fixed_rows_height() > content_top) {
                    ...
                    if (sb_init.position != 'undefined') {
                        ...
                        if (force || sb.css('position') != sb_init.position || INTEGRO_STORAGE['scroll_dir_last'] != scroll_dir || sb.width() != sb_init.width) {
    ...
    function integro_stretch_sidemenu() {
        ...
        var toc_items_height = integro_window_height() - integro_adminbar_height() - $menu_side_logo.outerHeight() - toc_items.length;
    ...
    $document.on('action.toc_menu_item_active', function() {
        ...
        var th = toc_items.eq(0).height(),
            ...
            menu_side_height = integro_window_height() - integro_adminbar_height() - $menu_side_logo.outerHeight() - $menu_side_logo.next('.toc_menu_item').outerHeight();
    ...
    function integro_resize_video(cont) {
        ...
        cont.find('video').each(function() {
            var $self = jQuery(this);
            if ((!INTEGRO_STORAGE['resize_tag_video'] && $self.parents('.mejs-mediaelement').length === 0) || $self.hasClass('trx_addons_resize') || $self.hasClass('trx_addons_noresize') || $self.parents('div:hidden,section:hidden,article:hidden').length > 0) {
                return;
            }
        ...
        if (INTEGRO_STORAGE['resize_tag_iframe']) {
            cont.find('.video_frame iframe,iframe').each(function() {
                var $self = jQuery(this);
                if ($self.hasClass('trx_addons_resize') || $self.hasClass('trx_addons_noresize') || $self.parent().is('rs-bgvideo') || $self.parents('rs-slide').length > 0 || $self.addClass('integro_resize').parents('div:hidden,section:hidden,article:hidden').length > 0) {
                    return;
                }
...
jQuery(document).on('action.init_hidden_elements', function(e, cont) {
    ...
    if (INTEGRO_STORAGE['button_hover'] && INTEGRO_STORAGE['button_hover'] != 'default') {
        jQuery('button:not(.search_submit):not(.full_post_close):not([class*="sc_button_hover_"]),' + '.sc_form_field button:not([class*="sc_button_hover_"]),' + '.theme_button:not([class*="sc_button_hover_"]),' + '.sc_button:not([class*="sc_button_simple"]):not([class*="sc_button_bordered"]):not([class*="sc_button_hover_"]),' + '.integro_tabs .integro_tabs_titles li a:not([class*="sc_button_hover_"]),' + '.post_item .more-link:not([class*="sc_button_hover_"]),' + '.trx_addons_hover_content .trx_addons_hover_links a:not([class*="sc_button_hover_"]),' + '#buddypress a.button:not([class*="sc_button_hover_"])' + '.mptt-navigation-tabs li a:not([class*="sc_button_hover_style_"]),' + '.edd_download_purchase_form .button:not([class*="sc_button_hover_style_"]),' + '.edd-submit.button:not([class*="sc_button_hover_style_"]),' + '.widget_edd_cart_widget .edd_checkout a:not([class*="sc_button_hover_style_"]),' + '.hover_shop_buttons .icons a:not([class*="sc_button_hover_style_"]),' + '.woocommerce #respond input#submit:not([class*="sc_button_hover_"]),' + '.woocommerce .button:not([class*="shop_"]):not([class*="view"]):not([class*="sc_button_hover_"]),' + '.woocommerce-page .button:not([class*="shop_"]):not([class*="view"]):not([class*="sc_button_hover_"])').addClass('sc_button_hover_just_init sc_button_hover_' + INTEGRO_STORAGE['button_hover']);
        ...
        jQuery('.sidebar .trx_addons_tabs .trx_addons_tabs_titles li a:not([class*="sc_button_hover_style_"]),' + '.integro_tabs .integro_tabs_titles li a:not([class*="sc_button_hover_style_"]),' + '.widget_tag_cloud a:not([class*="sc_button_hover_style_"]),' + '.widget_product_tag_cloud a:not([class*="sc_button_hover_style_"])').addClass('sc_button_hover_just_init sc_button_hover_style_alterbd');
...
(function() {
    "use strict";
    jQuery(document).on('action.add_googlemap_styles', integro_trx_addons_add_googlemap_styles).on('action.init_hidden_elements', integro_trx_addons_init);

    function integro_trx_addons_add_googlemap_styles(e) {
    ...
    function integro_trx_addons_init(e, container) {
        ...
        container.find('.sc_countdown_item canvas:not(.inited)').addClass('inited').attr('data-color', INTEGRO_STORAGE['alter_link_color']);
    }
})();