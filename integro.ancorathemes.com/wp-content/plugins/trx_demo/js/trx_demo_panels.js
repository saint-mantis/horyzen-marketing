...
function trx_demo_init() {
    ...
    $demo_panels.find('.trx_demo_tabs a:not([data-type="link"])').on('click', function(e) {
        var tab = jQuery(this),
            panels = tab.parents('.trx_demo_panels'),
            new_panel = panels.find(tab.attr('href'));
        if (!new_panel.hasClass('trx_demo_images_init')) {
            new_panel.addClass('trx_demo_images_init').find('[data-style^="background-image"]').each(function() {
    ...
    $demo_panels.find('.trx_demo_button_close,.trx_demo_panels_mask').on('click', function(e) {
        $demo_panels.removeClass('open').find('.trx_demo_panel_active').fadeOut(function() {
            jQuery(this).removeClass('trx_demo_panel_active');
        }).end().find('.trx_demo_tab_active').removeClass('trx_demo_tab_active');
        jQuery('html').removeClass('trx_demo_panels_open');
        e.preventDefault();
        return false;
    });
    $demo_panels.find('.trx_demo_panel_list_item[data-body-class] a').on('click', function(e) {
    ...
    $demo_panels.find('.trx_demo_panel_filters_list').on('click', 'a', function(e) {
    ...
    function filter_items(section, hide_all) {
        ...
        setTimeout(function() {
            ...
            section.find((flt != '' ? '[data-filter-value]' : '[data-search-value]') + (hide_all ? ':hidden' : '')).each(function() {
                ...
                if ((txt === '' || search_val.toLowerCase().indexOf(txt) >= 0) && (flt == 'all' || flt == '' || filter_compare(flt, flt_val.trim().toLowerCase()))) {
